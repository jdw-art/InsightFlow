...
class PromptManager:
    ...
    def render_rag_answer(
        ...
        """
        渲染 RAG 答案 Prompt（Prompt 内嵌 JSON Schema 方式）

        Args:
            schema: 答案类型 ('name' | 'number' | 'boolean' | 'names' | 'comparative')
            context: RAG 检索到的上下文内容
            question: 用户问题

        Returns:
            dict: {
                'system_prompt': str,  # 包含 instruction + JSON schema + example
                'user_prompt': str,   # 包含 context + question
            }
        """
        template_map = {
            'name': 'rag/name',
            ...
            'boolean': 'rag/boolean',
            'names': 'rag/names',
            'comparative': 'rag/comparative',
        }
        user_template_map = {
            'name': 'rag/user_rag',
            'number': 'rag/user_rag',
            'boolean': 'rag/user_rag',
            'names': 'rag/user_rag',
            'comparative': 'rag/user_comparative',
        }

        if schema not in template_map:
            raise ValueError(f"Unsupported RAG schema: {schema}")

        return {
            'system_prompt': self.render(template_map[schema]),
            'user_prompt': self.render(user_template_map[schema], context=context, question=question),
        }

    def render_rag_answer_name(self, context: str, question: str) -> Dict[str, str]:
        """渲染名称类问题 RAG Prompt"""
        return self.render_rag_answer('name', context, question)

    def render_rag_answer_number(self, context: str, question: str) -> Dict[str, str]:
        """渲染数值类问题 RAG Prompt"""
        return self.render_rag_answer('number', context, question)

    def render_rag_answer_boolean(self, context: str, question: str) -> Dict[str, str]:
        """渲染是/否类问题 RAG Prompt"""
        return self.render_rag_answer('boolean', context, question)

    def render_rag_answer_names(self, context: str, question: str) -> Dict[str, str]:
        """渲染多名称类问题 RAG Prompt"""
        return self.render_rag_answer('names', context, question)

    def render_rag_answer_comparative(self, context: str, question: str) -> Dict[str, str]:
        """渲染比较类问题 RAG Prompt"""
        return self.render_rag_answer('comparative', context, question)

    def render_rag_rerank_single(self, query: str, block: str) -> Dict[str, str]:
        """渲染单块 Reranking Prompt（返回 system/user 分离的 dict）"""
        template = self.env.get_template('rag/rerank.j2')
        return {
            'system_prompt': template.module.system_prompt(),
            'user_prompt': template.module.user_prompt(query=query, block=block),
        }

    def render_rag_rerank_multiple(self, query: str, blocks: List[str]) -> Dict[str, str]:
        """渲染多块 Reranking Prompt（返回 system/user 分离的 dict）"""
        template = self.env.get_template('rag/rerank_multiple.j2')
        return {
            'system_prompt': template.module.system_prompt(),
            'user_prompt': template.module.user_prompt(query=query, blocks=blocks),
        }

    def render_rephrased_questions(self, question: str, companies: str) -> Dict[str, str]:
        """渲染重述问题 Prompt（返回 system/user 分离的 dict）"""
        return {
            'system_prompt': self.render('rag/rephrased_questions'),
            'user_prompt': self.render('rag/user_rephrased_questions', question=question, companies=companies),
        }
